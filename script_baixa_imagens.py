from os import system
from time import sleep
import os

opcao = 0
while opcao != 9:
    print ("######################################")
    print ("#  Script para baixar boxes Vagrant  #")
    print ("#     Autor: Prof. Denys Alexandre   #")
    print ("#          Data: 15.12.2021          #")
    print ("######################################")
    print ("")
    print ("Escolha a opção que deseja")
    print ("")
    print ("(1) Listar todas as boxes disponíveis no repositório online;")
    print ("(2) Listar todas as boxes disponíveis no repositório local;")
    print ("(3) Baixar a box do Debian 11;")
    print ("(4) Baixar a box do Windows XP SP3;")
    print ("(5) Baixar a box do Windows 7 PRO;")
    print ("(6) Baixar a box do Windows 2008 Standard;")
    print ("(7) Baixar todas as boxes do repositório online (Isso vai demorar ... pega o café e espera)")
    print ("(8) Limpar a tela")
    print ("(9) Sair do script.")
    print ("")
    opcao =  int(input("O que você deseja fazer ? "))
    if opcao == 1:
        print('Listando todas as boxes disponíveis no repositório online')
        print ("")
        os.system('vagrant.exe cloud search denirow')
        sleep(3)
        print ("")
    elif opcao == 2:
        print('Listando todas as boxes disponíveis no repositório local')
        print ("")
        os.system('vagrant.exe box list')
        sleep(3)
        print ("") 
    elif opcao == 3:
        print('Baixando a box do Debian 11')
        print ("")
        os.system('vagrant.exe box add denirow/debian11')
        sleep(1)
    elif opcao == 4:
        print('Baixando a box do Windows XP SP3')
        print ("")
        os.system('vagrant.exe box add denirow/winxp')
        sleep(1)
    elif opcao == 5:
        print('Baixando a box do Windows 7 PRO')
        print ("")
        os.system('vagrant.exe box add denirow/win7pro')
        sleep(1)
    elif opcao == 6:
        print('Baixando a box do Windows 2008 Standard')
        print ("")
        os.system('vagrant.exe box add denirow/win2k8')
        sleep(1)
    elif opcao == 7:
        print('Baixando todas as boxes do repositório online (Isso vai demorar ... pega o café e espera)')
        print ("")
        print('Baixando a box do Debian 11')
        os.system('vagrant.exe box add denirow/debian11')
        print("")
        print("")
        print('Baixando a box do Windows XP SP3')
        os.system('vagrant.exe box add denirow/winxp')
        print("")
        print("")
        print('Baixando a box do Windows 7 PRO')
        os.system('vagrant.exe box add denirow/win7pro')
        print("")
        print("")
        print('Baixando a box do Windows 2008 Standard')
        os.system('vagrant.exe box add denirow/win2k8')
        print("")
        print("")
        sleep(1)
    elif opcao == 8:
        print('Limpando a tela')
        print ("")
        os.system('cls')
    elif opcao == 9:
        print('Finalizando ...')
        print("Saindo o script. Vlw. Flw.")
        sleep(2)
    else:
        print('Opção inválida. Tente novamente.')
        print ("")
        sleep(2)
